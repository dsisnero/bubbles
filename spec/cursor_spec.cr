...
describe Bubbles::Cursor do
  describe "Model" do
    it "creates a new model with default settings" do
      ...
      model.is_blinked?.should be_true
    ...
    it "handles blur message" do
      ...
      updated.is_blinked?.should be_true
    end
    ...
    it "toggles blink on valid blink message" do
      ...
      was_blinked = model.is_blinked?
      updated, cmd = model.update(blink_msg)
      updated.is_blinked?.should_not eq(was_blinked)
      cmd.should_not be_nil
    end
    ...
    it "ignores blink message with wrong id" do
      ...
      updated, cmd = model.update(blink_msg)
      cmd.should be_nil
      updated.is_blinked?.should eq(model.is_blinked?)
    end
  ...
  it "tests blink command data race" do
